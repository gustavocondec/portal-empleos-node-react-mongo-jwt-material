...
const Usuario = require("../models/modelosBD/Usuario")//importamos el modelo el usuario


userService.crearUsuario = async (CrearUsuarioModel) => {//recibe los datos correctos ya validados
    let emailUser = await Usuario.findOne({ email: CrearUsuarioModel.email });
    if (emailUser) {//si el correo y esta registrado
        return false;//es decir no se pude registrar la peticion
        ...
        newUser.password = await newUser.encryptPassword(CrearUsuarioModel.password)//encriptacion del password
        await newUser.save()
        return true;//representa que se registro correctamente
    }
}
...
userService.generarToken = async (IniciarSesionModel) => {
    let user = await Usuario.findOne({ email: IniciarSesionModel.email })//user es un documento
...
userService.actualizar = async function (ActualizarUsuarioModel, userId) {
    ...
    let { email, password, id, _id, tipo, ...resto } = ActualizarUsuarioModel;//retiramos los campos que no se pueden cambiar
    console.log("datos a guardar ", resto);
    const user = await Usuario.findByIdAndUpdate(userId, resto)
